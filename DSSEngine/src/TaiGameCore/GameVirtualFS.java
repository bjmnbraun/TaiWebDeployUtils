...
public class GameVirtualFS {
	...
	public String InputStreamTxtRead(InputStream is, String startAt) throws IOException{
	...
	public String getBaseJarUrl(){
		...
		if (whole.startsWith("jar")){
			...
			whole = whole.substring(0,the_1+1)+"2"+whole.substring(the_1+2);
		};
	...
	public URL getSrcUrlForString(String string, String base) throws IOException{
		URL srcU = null;
	...
	public PImage getImgFresh(final String string){
		...
		Thread made = new Thread(){public void run(){
			//System.out.println(srcU);
			try {
	...
	public TaiShaders loadShader(String string, GL2 gl){
		...
		String vertTxt=getStringResource("embed/gl/"+string+".vert","!!ARBvp1.0");
	...
	public void clearImages(){
		...
		while(!keySet.isEmpty()){
			...
			unloadResource(toRem.getValue(), toRem.getKey(),(PGraphicsOpenGL)toRem.getValue().parent.g);
		}
	}
