...
public class ProceGLHybrid extends KeyAdapter {
	...
	*/
	...
	public void viewport(Rectangle2D area) {
		viewport(area.getMinX(), area.getMinY(), area.getWidth(), area
				.getHeight());
	}
	...
	public void screen2D() {
		...
		//Copy the viewport:
		double[] oldViewport = new double[] { viewportStack[0],
				viewportStack[1], viewportStack[2], viewportStack[3] };
		viewport(0, 0, 1, 1);
		//Taken from PApplet.beginDraw
		gl.glMatrixMode(GL2.GL_PROJECTION);
		PMatrix3D projection = ((PGraphicsOpenGL) g.g).projection;
		float[] projectionFloats = new float[] { projection.m00,
				projection.m10, projection.m20, projection.m30, projection.m01,
				projection.m11, projection.m21, projection.m31, projection.m02,
				projection.m12, projection.m22, projection.m32, projection.m03,
				projection.m13, projection.m23, projection.m33 };
	...
	*/
	...
	public void handleInputAtBeginningOfDraw() {
		try {
			if (g.keyEvent == null) { //keyEvent is used for querying things that SHOULD be passed with keyTyped.
				...
				g.keyEvent = new KeyEvent(g, KeyEvent.KEY_PRESSED, System
						.currentTimeMillis(), 0, g.keyCode, '?');
			} else if (g.keyEvent.getID() == KEY_TYPED) {
				int[] remapping = new int[] { 10, VK_ENTER, 8, VK_BACK_SPACE,
						27, VK_ESCAPE, 9, VK_TAB, 127, VK_DELETE, 'z',
						KeyEvent.VK_Z, 'Z', KeyEvent.VK_Z, 'x', KeyEvent.VK_X,
						'X', KeyEvent.VK_X, };
	...
	public void outlineViewport() {
		g.stroke(0);
		g.noFill();
		g.rect(1f / currentViewPortWidth, 1f / currentViewPortHeight,
				1 - 2f / currentViewPortWidth, 1 - 2f / currentViewPortHeight);
	}
	...
	public String[] modSplit(String str, String regex, int endCaseW) {
		boolean hasExtra = str.substring(str.length() - endCaseW)
				.matches(regex);
