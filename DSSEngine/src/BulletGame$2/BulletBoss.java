...
public class BulletBoss extends BulletRelativitable {
	...
	public MultiExpression[] createExpression(Environment env, BulletGlobals bg)
		...
		return new MultiExpression[] { expr(x, env, bg), expr(y, env, bg),
				expr(rotate, env, bg), };
	}
	...
	public ArrayList<Exception> parseFromStrings(TaiDAWG<String> data,
		...
		while (iterator.hasNext()) {
			...
			if (!key.equals("x") && !key.equals("y") && !key.equals("rotate")
					&& !key.equals("w") && !key.equals("h")
	...
	public void autoWrittenDeSerializeCode() {
		String myGraphic_strTmp = ((StringEntry) readField("myGraphic",
	...
	public void autoWrittenSerializeCode() {
		writeField("myGraphic", new StringEntry(myGraphic != null ? myGraphic
				.hashToString() : ""));
