...
public abstract class BulletGame$1Engine$L3$2$BulletGameBenchmarks extends
		BulletGame$1Engine$L3$1$BulletGameEditorScreen {
	public BulletGame$1Engine$L3$2$BulletGameBenchmarks(JFrame holder,
			PApplet hold) {
		super(holder, hold);
	}
	...
	public class BenchmarkScreen extends BulletGameScreen {
		public BenchmarkScreen() {
			...
			for (int k = 0; k < myms.length; k++) {
				myms[k] = new MouseChecker(
						BulletGame$1Engine$L3$2$BulletGameBenchmarks.this);
			}
			g.registerDraw(this);
		...
		public void keyPressed(KeyEvent e) {
			if (!isInputBlocked()) {
				if (e.getKeyCode() == KeyEvent.VK_ESCAPE) {
					bgbs_loaded = null;
					SceneChange(Transitioner.FADE_WITH_BLACK,
							BulletHell.BulletGame$1Engine$GROUND.MAINSCREEN);
				}
			}
		}
		...
		private Runnable[] doOnStuff = new Runnable[] { new Runnable() {
			public void run() {
				if (hasCurrentDialog()) {
					return;
				}
			...
			public void run() {
				...
				SceneChange(
						Transitioner.FADE_WITH_BLACK,
						BulletHell.BulletGame$1Engine$GROUND.GAMESCREEN_FROMBENCHMARKS);
			}
		} };
		...
		public void drawScreen() {
			...
			if (bgbs_loaded != null) {
				Rectangle2D.Float area = new Rectangle2D.Float(.1f, .2f, .8f,
						.6f);
				viewport(area);
				g.fill(200, 200, 255);
				g.rect(0, 0, 1, 1);
		...
		private void drawLine(BulletGameBenchmark bg, float[] stuff,
				int length, float scale, float tScl) {
		...
		public class LoadBenchmarkDialog extends ModalDialog {
			...
			public LoadBenchmarkDialog(BulletGameScreen parent,
