...
public class DecodedVorbisAudioInputStream extends TAsynchronousFilteredAudioInputStream implements PropertiesContainer
{
	...
	private int bufferSize_ = bufferMultiple_ * 128 * 2;
	...
	private long currentBytesLastUpdate = System.nanoTime();
	 ...
	 * <li><b>ogg.position.byte.update</b> [Long], time at which we last wrote.
	 * <li><b>ogg.position.byte.updateSize</b> [Long], Size of buffer we are writing.
	 * </ul>
	 */
	public Map properties()
	{
		properties.put("ogg.position.byte",new Long(currentBytes));
		properties.put("ogg.position.byte.update",new Long(currentBytesLastUpdate==-1?System.nanoTime():currentBytesLastUpdate));
		properties.put("ogg.position.byte.updateSize",new Long(lbout));
		return properties;	 
	}
	/**
	 * Main loop.
	...
	public void execute()
	{
		...
		try{
			...
			case playState_WriteData:
				...
				break;
			case playState_BufferFull:
				numWritten += continueFromBufferFull();
				break;
			...
			if (numWritten>0){
				currentBytes = currentBytes + lbout;
				currentBytesLastUpdate = System.nanoTime();
				lbout = numWritten;
			}
		}
	}
	...
	private int continueFromBufferFull()
	{
		if(getCircularBuffer().availableWrite() < 2 * vorbisInfo.channels * bout)
		{
			if(TDebug.TraceAudioConverter) TDebug.out("Too much data in this data packet, better return, let the channel drain, and try again...");
