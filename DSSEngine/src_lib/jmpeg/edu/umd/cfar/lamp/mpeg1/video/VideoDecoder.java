...
public class VideoDecoder
{
	...
	{
		int pixelData[] = convertYCbCrToRGB(decoderState.getCurrentYCbCr(),null);
	...
	public VideoImage getImageOptimize(int[] output) throws IOException, MpegException{
		VideoImage toRet = new VideoImage();
		toRet.pixels = convertYCbCrToRGB(decoderState.getCurrentYCbCr(),output);
		toRet.width = getFrameWidth();
		toRet.height = getFrameHeight();
		return toRet;
	}

	private int[] convertYCbCrToRGB(int pixelData[], int[] result)
	{
		if (result==null){
			result = new int[pixelData.length];
		}
		if (result.length!=pixelData.length){
