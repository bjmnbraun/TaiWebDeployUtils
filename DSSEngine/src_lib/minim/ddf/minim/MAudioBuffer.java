...
final class MAudioBuffer implements AudioBuffer
{
  ...
  public synchronized void set(float[] buffer)
  {
    if (buffer.length != samples.length)
      Minim
          .error("MAudioBuffer.set: passed array must be the same length as this MAudioBuffer.");
    else
      samples = buffer;
  }
  ...
  public synchronized void mix(float[] b1, float[] b2)
  {
    if ((b1.length != b2.length)
      ...
      Minim
          .error("MAudioBuffer.mix: The two passed buffers must be the same size as this MAudioBuffer.");
    else
    {
