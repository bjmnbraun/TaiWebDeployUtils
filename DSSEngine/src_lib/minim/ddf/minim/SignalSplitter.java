...
public class SignalSplitter implements Recordable, AudioListener
{
  ...
  public synchronized void samples(float[] samp)
  {
    for (int i = 0; i < listeners.size(); i++)
    {
      ...
      float[] copy ;
      if (samples_copy!=null && samples_copy.length==samp.length){
    	  copy = samples_copy;
      } else {
    	  copy = new float[samp.length];  
    	  samples_copy = copy;
      }
  ...
  public synchronized void samples(float[] sampL, float[] sampR)
  {
    for (int i = 0; i < listeners.size(); i++)
    {
      AudioListener al = (AudioListener)listeners.get(i);
      if (copyL==null){
	      copyL = new float[sampL.length];
	      copyR = new float[sampR.length];
      }
